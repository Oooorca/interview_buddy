...
“设置 → 音频”中可以分别启用并选择麦克风和系统输出设备，也可以将“我的语言”和“对方语言”分别设为自动检测或不同的固定语言。
...
“设置 → 存储与清理”可以迁移数据、恢复默认目录、查看占用并安排安全缓存清理。使用自定义目录时，系统配置目录只保留一个很小的加密 `storage-location.secure.json` 引导文件。升级后会从旧标识符目录或 EXE 同目录的遗留 `cache` 复制受管理数据；确认新位置中的加密设置成功加载后，可以删除该遗留目录。
...
升级后首次启动时，有效的旧 `settings.json` 与 `storage-location.json` 会先完成加密、解密回读和校验，之后才删除明文副本。若密钥缺失、密文损坏或版本不受支持，应用不会退回明文，也不会覆盖原文件，而是进入锁定恢复页；只有明确确认重置后，才会隔离可定位文件并生成新密钥与默认设置。删除明文文件不等同于对 SSD 物理区块进行安全擦除。
