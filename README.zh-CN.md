...
“设置 → 通用”采用响应式双列布局，统一容纳语言、音频、存储和清理设置。界面语言和回答语言仍可分别控制：界面可跟随系统或固定为 `zh-CN`/`en-US`；回答可跟随界面或固定为其中一种语言。界面切换立即生效，保存后下次启动会继续使用；推荐默认 Prompt 跟随回答语言，自定义 Prompt 原样保留。转写语言仍在通用页的“音频输入与输出”区域独立配置，其中英文在设置中统一保存为 `en-US`，仅在发送请求时转换成服务商要求的代码。

没有语言字段的旧设置会迁移为中文，以保持升级前行为；新安装默认跟随系统，回答跟随界面。Windows 安装器会在中文和英文之间跟随系统语言，macOS 权限说明也提供两种语言。
...
旧版空值和已知历史默认值会迁移为“推荐默认”，其他已有内容会保留为“自定义”。
...
“设置 → 通用 → 存储与清理”可以迁移数据、恢复默认目录、查看占用并安排安全缓存清理。使用自定义目录时，系统配置目录只保留一个很小的加密 `storage-location.secure.json` 引导文件。升级后会从旧标识符目录或 EXE 同目录的遗留 `cache` 复制受管理数据；确认新位置中的加密设置成功加载后，可以删除该遗留目录。
